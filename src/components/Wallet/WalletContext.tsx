...
import type { SuiTransactionBlockResponse, SuiTransactionBlockResponseQuery, PaginationArguments, OrderArguments } from '@mysten/sui/client';
...
export function WalletProvider(props: { children: JSXElement; }) {
    ...
    const loadSavedState = () => {
        try { const savedState = localStorage.getItem('walletState'); if (savedState) { return JSON.parse(savedState); } }
        catch (error) { console.error('Error loading saved wallet state:', error); }
        return {};
    };
    ...
    onMount(() => {
        ...
        const unsubscribe = walletAdapter.on('change' as any, (event: { wallets: readonly Wallet[]; }) => {
            setState("availableWallets", event.wallets);
        });
        const walletName = savedWalletName();
        if (walletName && initialWallets.some(w => w.name === walletName)) { setTimeout(() => { connect(walletName); }, 500); }
        return unsubscribe;
    });

    createEffect(() => {
        let accountToSave = null;
        if (state.activeAccount) { accountToSave = { address: state.activeAccount.address }; }
        const stateToSave = {
            activeAccount: accountToSave,
            walletName: state.selectedWallet?.name,
            network: state.network
        };
        try { localStorage.setItem('walletState', JSON.stringify(stateToSave)); }
        catch (error) { console.error('Error saving wallet state:', error); }
    });
    ...
    const fetchTransactionHistory = async (options?: { force?: boolean; }) => {
        ...
        setState(produce(s => { s.isHistoryLoading = true; s.historyError = null; }));
        ...
        try {
            ...
            const sortedHistory = historyResponse.data.sort((a, b) => parseInt(b.timestampMs || '0') - parseInt(a.timestampMs || '0'));
            setState("transactionHistory", sortedHistory);
        } catch (error) {
            console.error("Error fetching transaction history:", error);
            setState(produce(s => { s.historyError = error instanceof Error ? error.message : "Unknown error"; s.transactionHistory = []; }));
        } finally {
    ...
    const refreshBalances = async (): Promise<void> => {
        if (state.activeAccount && state.isConnected) {
            try {
                ...
                const suiBalance = { symbol: "SUI", name: "Sui", balance: ownerBalance.totalBalance, decimals: 9 };
                const ownedObjects = await client.getOwnedObjects({ owner: address, options: { showType: true } });
                ...
                setState({
                    balances: [suiBalance],
                    collections: { encryptedDrives: encryptedDrivesCount, decryptedPrograms: decryptedProgramsCount }
                });
            } catch (error) { console.error("Error refreshing balances/collections:", error); }
        }
    };

    const connect = async (walletName?: string): Promise<boolean> => {
        setState(produce(s => {
            s.isConnecting = true; s.error = null; s.transactionHistory = []; s.historyError = null;
        ...
        try {
            ...
            const connectFeature = walletToConnect.features['standard:connect'] as { connect: () => Promise<void>; } | undefined;
            if (!connectFeature) throw new Error(`${walletToConnect.name} doesn't support connect feature.`);
            ...
            if (walletToConnect.accounts.length > 0) {
                ...
                const eventsFeature = walletToConnect.features['standard:events'] as { on: (event: 'change', listener: (params: { accounts?: readonly WalletAccount[]; }) => void) => () => void; } | undefined;
                if (eventsFeature) {
                    eventsFeature.on('change', ({ accounts }: { accounts?: readonly WalletAccount[]; }) => {
                        ...
                        } else { console.log("Wallet accounts empty via event."); disconnect(); }
                    });
                }
                return true;
            } else {
            ...
            setState(produce(s => {
                ...
                s.isConnected = false; s.selectedWallet = null; s.activeAccount = null;
            }));
            return false;
        } finally {
    ...
    const disconnect = async (): Promise<void> => {
        if (state.selectedWallet && state.isConnected) {
            try {
                const disconnectFeature = state.selectedWallet.features['standard:disconnect'] as { disconnect: () => Promise<void>; } | undefined;
    ...
    const switchNetwork = (requestedNetwork: NetworkType): void => {
        ...
        if (state.isConnected) {
            disconnect().then(() => {
                console.log("Disconnected due to network switch. Please reconnect if previously connected.");
            });
        }
        // Persist the new network choice immediately
        try {
            const currentState = JSON.parse(localStorage.getItem('walletState') || '{}');
            localStorage.setItem('walletState', JSON.stringify({ ...currentState, network: targetNetwork }));
        } catch (error) {
