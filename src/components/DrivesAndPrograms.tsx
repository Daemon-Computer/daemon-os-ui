...
export default function DrivesAndPrograms() {
    ...
    const [statusType, setStatusType] = createSignal<'info' | 'success' | 'error' | 'warning'>('info');
    ...
    const checkRequiredIds = (): boolean => {
        const areSet = DAEMON_PACKAGE_ID && !DAEMON_PACKAGE_ID.includes("PLACEHOLDER") &&
            DRIVE_MINTER_OBJECT_ID && !DRIVE_MINTER_OBJECT_ID.includes("PLACEHOLDER") &&
            PROGRAM_MINTER_OBJECT_ID && !PROGRAM_MINTER_OBJECT_ID.includes("PLACEHOLDER") &&
            PROGRAM_REGISTRY_OBJECT_ID && !PROGRAM_REGISTRY_OBJECT_ID.includes("PLACEHOLDER") &&
            DISTRIBUTION_TABLE_OBJECT_ID && !DISTRIBUTION_TABLE_OBJECT_ID.includes("PLACEHOLDER");

        setConfigValid(areSet);
    ...
    const fetchShopConfig = async () => {
        if (!checkRequiredIds()) { setIsConfigLoading(false); return; }
        if (!state.isConnected) { setIsConfigLoading(false); return; }
        ...
        try {
            ...
            if (minterObject.data?.content?.dataType === 'moveObject') {
                ...
                setShopConfig({ priceMist, priceSui, isMinterEnabled: isEnabled });
            } else {
    ...
    const fetchUserDrives = async () => {
        ...
        try {
            ...
            const drives: UserDrive[] = ownedObjects.data
                .map((obj) => {
                    if (obj.data?.type === driveType && obj.data?.content?.dataType === 'moveObject' && obj.data.content.fields) {
                        return {
    ...
    const buyEncryptedDrive = async () => {
        ...
        try {
            ...
            const signAndExecute = state.selectedWallet.features['sui:signAndExecuteTransactionBlock'] as SignAndExecuteMethodHolder;
            if (!signAndExecute) throw new Error('Wallet does not support signAndExecuteTransactionBlock');
            ...
            } else if (errorMessage.includes("IncorrectSigner") || errorMessage.includes("Signature validation failed")) {
                    ...
                    <div class="flex-1 flex flex-col items-center justify-center p-4">
                        ...
                        >
                            {isLoading() ? 'Purchasing...' :
                                shopConfig().priceSui ? `Buy for ${shopConfig().priceSui} SUI` : 'Loading price...'}
                        </button>
