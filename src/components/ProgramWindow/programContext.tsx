...
interface ProgramRegistry {
    registerProgram: (program: Omit<RunningProgram, 'id' | 'zIndex' | 'x' | 'y' | 'minimizeTargetX' | 'minimizeTargetY' | 'isMinimized' | 'width' | 'height'> & { initialWidth: number, initialHeight: number; }) => string;
...
export function ProgramProvider(props: { children: JSXElement; }) {
    ...
    const normalizeZIndices = (currentPrograms: RunningProgram[]) => {
        ...
        currentPrograms.filter(p => p.isMinimized).forEach(p => { p.zIndex = 0; });
        return currentZ - 1;
    };

    const registerProgram = (programDetails: Omit<RunningProgram, 'id' | 'zIndex' | 'x' | 'y' | 'minimizeTargetX' | 'minimizeTargetY' | 'isMinimized' | 'width' | 'height'> & { initialWidth: number, initialHeight: number; }) => {
        const id = crypto.randomUUID();

        const initialX = Math.max(0, (window.innerWidth - programDetails.initialWidth) / 2);
        const initialY = Math.max(0, (window.innerHeight - TASKBAR_HEIGHT_PX - programDetails.initialHeight) / 2);

        setPrograms(produce(currentPrograms => {
