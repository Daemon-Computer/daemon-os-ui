...
function mapPartNameToEnum(name: string): ProgramPartName {
  ...
  switch (name) {
    // Robot Body parts
    case "BoxRobotBody":
      return ProgramPartName.BoxRobotBody;
    case "RoundRobotBody":
      return ProgramPartName.RoundRobotBody;

    // Robot Head parts
    case "BoxRobotHead":
      return ProgramPartName.BoxRobotHead;
    case "PyramidRobotHead":
      return ProgramPartName.PyramidRobotHead;
    case "SphereRobotHead":
      return ProgramPartName.SphereRobotHead;

    // Robot Eye parts
    case "CameraRobotEye":
      return ProgramPartName.CameraRobotEye;
    case "FeelerRobotEye":
      return ProgramPartName.FeelerRobotEye;
    case "SleekCameraRobotEye":
      return ProgramPartName.SleekCameraRobotEye;

    // Robot Addon parts
    case "AntennaRobotAddon":
      return ProgramPartName.AntennaRobotAddon;

    // Robot Leg Base parts
    case "HexagonalRobotLegBase":
      return ProgramPartName.HexagonalRobotLegBase;
    case "CrownedRobotLegBase":
      return ProgramPartName.CrownedRobotLegBase;

    // Robot Leg Tibia parts
    case "StraightRobotLegTibia":
      return ProgramPartName.StraightRobotLegTibia;
    case "MiddleRobotLegTibia":
      return ProgramPartName.MiddleRobotLegTibia;
    case "SideRobotLegTibia":
      return ProgramPartName.SideRobotLegTibia;

    // Robot Leg Joint parts
    case "HexagonalRobotLegJoint":
      return ProgramPartName.HexagonalRobotLegJoint;
    case "CrossRobotLegJoint":
      return ProgramPartName.CrossRobotLegJoint;
    case "StarRobotLegJoint":
      return ProgramPartName.StarRobotLegJoint;

    // Robot Leg End parts
    case "SideRobotLegEnd":
      return ProgramPartName.SideRobotLegEnd;
...
function transformSuiPaletteToWasmPalette(
  ...
  if (
    ...
    console.warn("WASM Palette Prep: SUI palette is missing or core fields (primary, accent, highlight) are invalid. Cannot use SUI palette.");
    return null;
  }

  // Validate that the fields contain proper HSV values
  const validateColor = (color: any, name: string): boolean => {
    if (!color || typeof color.hue !== 'number' || typeof color.saturation !== 'number' || typeof color.value !== 'number') {
  ...
  if (!validateColor(suiPalette.primary.fields, 'primary') ||
  ...
  const result = {
    ...
    background:
      suiPalette.background?.fields || DEFAULT_MINI_PALETTE.background,
  };
...
function transformSuiPaletteToViewModelPalette(
  ...
  const scale = (val: number | undefined) =>
    typeof val === "number" ? val / 255 : 0.5;
  const scaleHue = (val: number | undefined) =>
    typeof val === "number" ? val / 360 : 0;
  ...
  const neutralColorFields =
    suiPalette.neutral?.fields || defaultSuiColorFields;
  const backgroundColorFields =
    suiPalette.background?.fields || defaultSuiColorFields;
...
function transformSuiPartsToWasmParts(
  ...
  return suiParts.map((part) => {
    ...
    const lowerName =
      typeof part.name === "string" ? part.name.toLowerCase() : "";

    if (lowerName.includes("body")) partName = ProgramPartName.SimpleBody;
    else if (lowerName.includes("limb")) partName = ProgramPartName.SimpleLimb;
    else if (lowerName.includes("eye")) partName = ProgramPartName.SimpleEye;
...
function transformSuiPartsToViewModelAddons(suiParts: any): ViewModelAddon[] {
  ...
  return suiParts.map((part, index) => {
    ...
    if (typeof partName === "string" && partName.length > 0) {
      ...
      console.warn(
        `Part at index ${index} has invalid or missing name:`,
        partName,
      );
    }
    // ---------------------------------------------------------------------
    ...
    if (partParams && partParams.length > 0) {
      if (addonKey === "Lizard" || addonKey === "SimpleLimb") {
        if (partParams.length > 0)
          addon[addonKey]!.size = ((partParams[0] % 2000) + 500) / 1000;
        ...
        if (partParams.length > 0)
          addon[addonKey]!.end = ((partParams[0] % 500) + 100) / 1000;
        if (partParams.length > 1)
          addon[addonKey]!.length = (partParams[1] % 10) + 2;
      } else {
        // Generic fallback
        if (partParams.length > 0)
          addon[addonKey]!.size = ((partParams[0] % 2000) + 500) / 1000;
      ...
      console.warn(
        `Part "${addonKey}" at index ${index} has missing or empty params array.`,
      );
    }
...
function getProtocolNameFromSui(
...
function transformRawPartsForWasm(
  ...
  for (let i = 0; i < rawParts.length; i++) {
    ...
    if (partNameEnum === ProgramPartName.SimpleBody ||
      partNameEnum === ProgramPartName.BoxRobotBody ||
      partNameEnum === ProgramPartName.RoundRobotBody) {
      hasBody = true;
    }

    // Fix Empty parts and SimpleEye to have no parameters (matching working ProgramEventDemo format)
    if (partNameEnum === ProgramPartName.Empty || partNameEnum === ProgramPartName.SimpleEye) {
      partParams = [];
    }
  ...
  if (!hasBody) {
    console.warn(`WASM Prep (${programId}): No body part found in SUI data. Adding SimpleBody as first part.`);
  ...
  console.log(`WASM Parts Prep: Successfully transformed ${transformedParts.length} parts:`, transformedParts);
...
export default function ProgramDatabase() {
  ...
  const [selectedProgram, setSelectedProgram] = createSignal<Program | null>(
    null,
  );
  const [viewerBridge, setViewerBridge] =
    createSignal<WasmCanvasBridgeInterface | null>(null);
  ...
  onMount(() => {
    ...
    const handleWebGPUError = (event: any) => {
    ...
    try {
      ...
      for (const obj of ownedProgramObjects.data) {
        if (obj.data && obj.data.content?.dataType === "moveObject") {
          const fields = obj.data.content
            .fields as unknown as SuiProgramDataFields;
          ...
          const programName =
            ...
            `Program ${obj.data.objectId.slice(-6)}`;

          const randomSpeed =
            (parseInt(obj.data.objectId.slice(-2, -1), 16) % 5) + 1;
          const randomCorruption =
            (parseInt(obj.data.objectId.slice(-3, -2), 16) % 80) + 10;
          const randomMaxHealth =
            (parseInt(obj.data.objectId.slice(-4, -3), 16) % 50) + 50;
          const randomDamage =
            (parseInt(obj.data.objectId.slice(-5, -4), 16) % 25) + 5;
          ...
          if (timestampFromHistory) {
            ...
            if (creationTimestampFromDisplay) {
              if (!isNaN(Number(creationTimestampFromDisplay))) {
                ...
                try {
                  mintDateStr = new Date(
                    creationTimestampFromDisplay,
                  ).toLocaleDateString();
                } catch (e) {
                  /* Ignore parsing error, keep N/A */
                }
              } // Else: keep N/A if display data is unusable
              // if (mintDateStr !== "N/A") console.log(`Found mint date for ${obj.data.objectId} from display object: ${mintDateStr}`);
            }
            // If display object also fails, mintDateStr remains "N/A"
            // if (mintDateStr === "N/A") console.log(`Could not determine mint date for ${obj.data.objectId} from history or display.`);
          }
          // --- End Mint Date Logic ---

          // First, create ViewModel for UI display
          const viewModelPalette = transformSuiPaletteToViewModelPalette(
            fields.palette,
          );
      ...
      console.log(
        `Successfully transformed ${fetchedProgramData.length} programs.`,
      );
    } catch (e) {
  ...
  const displayData = () =>
    selectedProgram() ?? DEFAULT_PROGRAM_DATA_PLACEHOLDER;
    ...
    if (bridge?.isReady() && currentProgram) {
      try {
        ...
        if (currentProgram.rawFields?.palette) {
          suiPaletteProcessed = transformSuiPaletteToWasmPalette(
            currentProgram.rawFields.palette,
          );
        }
        ...
        console.log(`Sending data to WASM for program ${programId}:`, JSON.stringify(programBuilder, null, 2));
        try {
          bridge.queueEventForWasm({ ViewModel: programBuilder as any });
          console.log(
            `Successfully sent ViewModel for program ${programId} to WASM`,
          );
          ...
          setWebGPUError(
            "Failed to communicate with 3D renderer. Please refresh the page.",
          );
        }
      } catch (error) {
        console.error(
  ...
  function generatePaletteFromId(id: string): any {
    ...
    return {
      primary: createMiniColor(
        byte1 % 360,
        150 + (byte2 % 105),
        180 + (byte3 % 75),
      ),
      accent: createMiniColor(
        (byte2 + 120) % 360,
        150 + (byte3 % 105),
        150 + (byte4 % 105),
      ),
      highlight: createMiniColor(
        (byte3 + 240) % 360,
        180 + (byte4 % 75),
        200 + (byte5 % 55),
      ),
      neutral: createMiniColor(
        byte4 % 30,
        20 + (byte5 % 80),
        180 + (byte1 % 75),
      ),
      background: createMiniColor(
        (byte5 + 180) % 360,
        100 + (byte1 % 80),
        150 + (byte2 % 75),
      ),
  ...
  function generatePartsFromId(id: string): any[] {
    ...
    return [
      ...
      {
        name:
          byte4 % 2 === 0 ? ProgramPartName.SimpleLimb : ProgramPartName.Empty,
        params:
          byte4 % 2 === 0 ? [1 + (byte5 % 2)] : [], // Empty parts get no params
      },
      {
        name:
          byte5 % 3 === 0 ? ProgramPartName.SimpleLimb : ProgramPartName.Empty,
        params:
          byte5 % 3 === 0 ? [1] : [], // Empty parts get no params
      },
    ];
  }
    ...
    <div class="flex w-full h-full pt-2 overflow-hidden">
              ...
              <div class="p-4 text-center text-red-500 border border-red-500 flex-1 flex items-center justify-center">
                <div>
                  ...
                  <p class="mt-2 text-sm">
                    Please use a recent version of Chrome, Edge, or enable flags
                    in Firefox.
                  </p>
                </div>
              </div>
          ...
          <div class="overflow-hidden">
            <div class="flex justify-between status-bar-field px-2 items-center">
              ...
              <div class="flex gap-2 text-gray-500 mr-2 text-xs truncate">
                Decrypted:{" "}
                <p class="italic truncate">{displayData().mintDate}</p>
              </div>
            </div>
            <div class="mt-4 text-sm">
              ...
              <Show
                when={selectedProgram() && selectedProgram()?.rawFields?.parts}
              >
                <div class="flex justify-between text-blue-600 mt-2">
                  <div>3D Model</div>
                  <div class="truncate">
                    Legacy Parts (
                    {selectedProgram()?.rawFields?.parts?.length || 0})
                  </div>
                </div>
              </Show>
            </div>
                  ...
                  <div class="flex-col h-full text-right">
                    <Show
                      when={
                        selectedProgram() &&
                        displayData().name !==
                        DEFAULT_PROGRAM_DATA_PLACEHOLDER.name
                      }
                      fallback={
                        <>
      ...
      <div class="w-3/5 flex flex-col ml-1 mr-2 min-w-0 overflow-hidden">
        ...
        <div class="overflow-y-auto overflow-x-hidden p-1">
          ...
          <Show
            when={
              !isLoading() && programs().length === 0 && walletState.isConnected
            }
          >
