...
declare global {
  interface Window {
    ...
    _wasmBridgeFuncs?: Record<
      string,
      { poll: () => EventPayload | 'Empty'; receive: (event: EventPayload) => void }
    >;
...
function setupGlobalEventDelegates() {
  ...
  if (typeof window.wasm_event === 'undefined') {
    window.wasm_event = () => {
      ...
      console.log(
        `[Global window.wasm_event] Called. Current poller is ${typeof poller === 'function' ? 'set' : 'NOT SET'}.`,
      ); // Added log
  ...
  if (typeof window.web_event === 'undefined') {
    window.web_event = (event: EventPayload) => {
      ...
      console.log(
        `[Global window.web_event] Called with event: ${JSON.stringify(event)}. Current receiver is ${typeof receiver === 'function' ? 'set' : 'NOT SET'}.`,
      ); // Added log
...
export function createWasmCanvas(options: CreateWasmCanvasOptions): [
  ...
  function setupVisibilityHandling(canvas: HTMLCanvasElement, _wasmInstance: any) {
    ...
    const handleVisibilityChange = () => {
      console.log(
        '[' + uniqueId + '] Page visibility: ' + (document.hidden ? 'hidden' : 'visible'),
      );
      // Ideal place to call pause/resume on wasmInstance if available
  ...
  async function initialize(canvas: HTMLCanvasElement, bridgeInstance: GameBridge) {
    ...
    if (typeof window._currentWasmEventPoller !== 'undefined') {
      console.warn('[' + uniqueId + '] _currentWasmEventPoller was already defined! Overwriting.');
    }
    if (typeof window._currentWebEventReceiver !== 'undefined') {
      console.warn('[' + uniqueId + '] _currentWebEventReceiver was already defined! Overwriting.');
    }
    ...
    try {
      ...
      console.log(
        '[' + uniqueId + '] Initialize FINALLY block. Ready=' + isReady() + ', Error=' + error(),
      );
    }
  }
  ...
  const bridgeInterfaceMemo = createMemo(
    (): WasmCanvasBridgeInterface | null => {
    ...
    null,
    { equals: false },
  ); // equals: false ensures updates if the object identity changes
