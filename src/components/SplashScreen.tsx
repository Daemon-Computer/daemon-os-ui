...
export default function SplashScreen(props: SplashScreenProps) {
  ...
  const [revealedLines, setRevealedLines] = createSignal<boolean[]>(
    new Array(ASCII_LOGO.length).fill(false),
  );
    ...
    preloadAllAssets((status: AssetStatus) => {
      ...
      .then(() => {
        ...
        setStatusMessages((prev) => [
          ...
          {
            id: Date.now() + prev.length,
            text: 'System Online. Welcome to Daemon OS.',
            timestamp: Date.now(),
          },
        ]);
      ...
      .catch((error) => {
        ...
        setStatusMessages((prev) => [
          ...
          {
            id: Date.now(),
            text: `CRITICAL ERROR: ${error.message}. Attempting to continue...`,
            timestamp: Date.now(),
            isError: true,
          },
        ]);
  ...
  return (
    <Show when={isVisible()}>
      ...
      >
        ...
        <div
          class="fixed bottom-0 left-0 w-full p-4 overflow-hidden h-48"
          style={{ 'z-index': '3' }}
        >
              ...
              {(message, indexSignal) => {
                const itemData = createMemo(() => {
                  ...
                  const transformOffset =
                    totalDisplayed > 0 ? totalDisplayed - 1 - displayIndex : 0;
                  return {
