...
async function preloadWasm(statusCallback: StatusCallback): Promise<void> {
    ...
    try {
        ...
        if (!headResponse.ok) {
            statusCallback({ message: `Engine HEAD request failed: ${headResponse.statusText} (${headResponse.status}). Will try cache.`, error: true });
        }
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown network error';
        statusCallback({ message: `Engine HEAD request network error: ${errorMessage}. Will try cache.`, error: true });
    }

    const currentEtag = headResponse?.headers.get('ETag') || headResponse?.headers.get('Last-Modified');
    ...
    if (headResponse?.ok && currentEtag && currentEtag === cachedEtag) {
        ...
        statusCallback({ message: 'Engine cache miss despite ETag match. Re-fetching...', error: true }); // Considered an error if ETag matched but not in cache
    } else if (headResponse?.ok) {
        statusCallback({ message: cachedEtag ? 'Engine update detected. Fetching new version...' : 'Fetching engine for the first time...' });
    } else {
    ...
    try {
        ...
        statusCallback({ message: `Engine fetch error: ${errorMessage}. Trying cache as fallback.`, error: true });
        const cachedResponse = await cache.match(WASM_URL);
        if (cachedResponse) {
...
async function preloadPublicAsset(asset: AssetDefinition, statusCallback: StatusCallback): Promise<void> {
    ...
    if (asset.url === JS_BINDINGS_URL) {
        ...
        try {
            ...
            if (!headResponse.ok) {
                statusCallback({ message: `HEAD request for ${asset.name} failed: ${headResponse.statusText} (${headResponse.status}). Will try cache.`, error: true });
            }
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown network error';
            statusCallback({ message: `HEAD request for ${asset.name} network error: ${errorMessage}. Will try cache.`, error: true });
        }
        ...
        if (headResponse?.ok && currentEtag && currentEtag === cachedEtag) {
            ...
            statusCallback({ message: `${asset.name} cache miss despite ETag match. Re-fetching...`, error: true });
        } else if (headResponse?.ok) {
            statusCallback({ message: cachedEtag ? `${asset.name} update detected. Fetching new version...` : `Fetching ${asset.name} for the first time...` });
        } else {
        ...
        try {
            ...
            const errorMessage = fetchError instanceof Error ? fetchError.message : 'Unknown fetch error';
            statusCallback({ message: `${asset.name} fetch error: ${errorMessage}. Trying cache as fallback.`, error: true });
            const cachedResponse = await cache.match(asset.url);
            if (cachedResponse) {
                statusCallback({ message: `${asset.name} loaded from cache (network fetch failed).` });
            } else {
                statusCallback({ message: `CRITICAL: ${asset.name} fetch failed and not in cache.`, error: true });
                throw new Error('Failed to load WASM from network and cache.'); // Re-throw critical error
            }
        }
    } else {
        ...
        try {
            ...
            if (!response.ok) {
                statusCallback({ message: `Failed to fetch ${asset.name}: ${response.statusText} (${response.status})`, error: true });
            } else {
